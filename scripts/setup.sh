...
# Download and extract PRG data
if [[ ! -f /tmp/resources/jednostki_administracyjne.zip ]]; then
    wget -P /tmp/resources ftp://91.223.135.109/prg/jednostki_administracyjne.zip
fi
unzip /tmp/resources/jednostki_administracyjne.zip -d /opt/geoserver/prg

# Build geogig and other community modules

if  [ "$COMMUNITY_MODULES" == true ]; then
    array=(geoserver-${GS_VERSION:0:4}-SNAPSHOT-s3-geotiff-plugin.zip)
    for i in "${array[@]}"
    do
